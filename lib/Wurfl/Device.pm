...
sub id {
    ...
    return Wurfl::wurfl_device_get_id($self->{_device});
}

sub useragent {
    my ($self) = @_;
    return Wurfl::wurfl_device_get_useragent($self->{_device});
}

sub get_capability {
    my ($self, $capability) = @_;
    my $val = Wurfl::wurfl_device_get_capability($self->{_device}, $capability);
    return $CONVERT_BOOLEAN{$val} // $val;
}

sub has_capability {
    my ($self, $capability) = @_;
    return Wurfl::wurfl_device_has_capability($self->{_device}, $capability);
}

sub has_virtual_capability {
    my ($self, $capability) = @_;
    return Wurfl::wurfl_device_has_virtual_capability($self->{_device}, $capability);
}

sub get_virtual_capability {
    my ($self, $capability) = @_;
    my $val = Wurfl::wurfl_device_get_virtual_capability($self->{_device}, $capability);
    return $CONVERT_BOOLEAN{$val} // $val;
}

sub match_type {
    my ($self) = @_;
    return Wurfl::wurfl_device_get_match_type($self->{_device});
}

sub matcher_name {
    my ($self) = @_;
    return Wurfl::wurfl_device_get_matcher_name($self->{_device});
}

sub DESTROY {
    my $self = shift;

    Wurfl::wurfl_device_destroy($self->{_device});
}

=head1 NAME

Wurfl::Device - Device perl wrapper to wurfl_device_handle
...
Returns the id of the current device. The id can be used later with Wurfl::get_device()
...
Andrea Guzzo, E<lt>aguzzo@prod.lhr1.booking.comE<gt>
